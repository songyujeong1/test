  ...
  methods: {
    onHandleClick() {
      ...
      fileterRow.map(item => {
        if (has(item, 'bookingId') && !this.isEmpty(item.bookingId) && !this.isEmpty(item.calcDetailState)) {
          this.bookingInfoList.push({'bookingId': item.bookingId, 'calcDetailState': item.calcDetailState});
        } else if (has(item, 'bookingNo') && !this.isEmpty(item.bookingNo) && !this.isEmpty(item.calcDetailState)) {
          this.bookingInfoList.push({'bookingId': item.bookingNo, 'calcDetailState': item.calcDetailState});
        }
      });
      ...
      if (this.getPageName() === 'psalescalc' || this.getPageName() === 'pcancelfeecheck') {
        this.openPopup();
      } else {
        this.confirmMsg();
      }
    ...
    confirmMsg() {
      ...
      messageBox.show('', confirmMsg, 'YesNo').then(dialog => {
        if (dialog === 'yes') {
          this.calcProcess(this.calcType, uniq(this.bookingInfoList), this.calcRevenueYearMonth, (this.getPageName() === 'pcancelfeecheck') ? 'Y' : 'N');
          this.onHandleClose();
        }
      });
    },
