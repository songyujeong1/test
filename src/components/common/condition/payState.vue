...
<div class="col-md-1" :style="getStyle()">
  <div class="form-group">
    <label>기안상태</label>
    <el-select v-model="value.payState" placeholder="전체" size="small" @change="onHandleDataChange" clearable>
...
export default {
  ...
  mounted() {
    if (this.isInitializeValue === true && this.field && this.field.defaultValue) {
      Object.assign(this.value, {payState: this.field.defaultValue});
    }
  ...
  methods: {
    initializeComponent() {
      this.getDetailCodeList('9340').then((payload) => {
