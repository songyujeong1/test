...
<div class="col-md-1" :style="getStyle()">
  <div class="form-group">
    <label>거래유형</label>
    <el-select v-model="value.sprType" placeholder="선택" size="small" @change="onHandleDataChange">
...
export default {
  ...
  mounted() {
    if (this.isInitializeValue === true && this.field && this.field.defaultValue) {
      Object.assign(this.value, {sprType: this.field.defaultValue});
    }
  ...
  methods: {
    initializeComponent() {
      this.getDetailCodeList('9620').then((payload) => {
