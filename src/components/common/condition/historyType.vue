...
<div class="col-md-1" :style="getStyle()">
  <div class="form-group">
    <label>변경이력 유형</label>
    <el-select v-model="value.historyType" placeholder="전체" size="small" @change="onHandleDataChange" clearable>
...
export default {
  ...
  mounted() {
    if (this.isInitializeValue === true && this.field && this.field.defaultValue) {
      Object.assign(this.value, {historyType: this.field.defaultValue});
    }
    this.initializeComponent();
  },
  methods: {
    async initializeComponent() {
      this.options = await this.getDetailCodeListAsync('9380');
    }
  }
};
</script>
